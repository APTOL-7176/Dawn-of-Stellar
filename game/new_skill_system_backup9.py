...
def apply_special_effect(effect_name: str, caster, target=None, skill_data=None):
    ...
    effects_map = {
        ...
        # 여섯 번째 배치 효과들
        "double_strike": lambda: _double_strike(caster, target, skill_data),
        "draconic_might": lambda: _draconic_might(caster),
        "earth_elementalist": lambda: _earth_elementalist(caster),
        "earth_power": lambda: _earth_power(caster),
        "earth_resonance": lambda: _earth_resonance(caster),
        "electric_boost": lambda: _electric_boost(caster),
        "electric_field": lambda: _electric_field(caster),
        "elemental_armor": lambda: _elemental_armor(caster),
        "elemental_barrier": lambda: _elemental_barrier(caster),
        "elemental_mastery": lambda: _elemental_mastery(caster),
        "elemental_overload": lambda: _elemental_overload(caster),
        "energy_absorption": lambda: _energy_absorption(caster, target, skill_data),
        "energy_boost": lambda: _energy_boost(caster),
        "energy_focus": lambda: _energy_focus(caster),
        "energy_overload": lambda: _energy_overload(caster),
        "energy_recharge": lambda: _energy_recharge(caster),
        "enhanced_accuracy": lambda: _enhanced_accuracy(caster),
        "enhanced_luck": lambda: _enhanced_luck(caster),
        "enhanced_reflexes": lambda: _enhanced_reflexes(caster),
        "eternal_flame": lambda: _eternal_flame(caster),
        # 다섯 번째 배치 효과들
        "chronos_blessing": lambda: _chronos_blessing(caster),
        "combo_mark": lambda: _combo_mark(caster, target, skill_data),
        "combo_multiplier": lambda: _combo_multiplier(caster),
        "combo_strike": lambda: _combo_strike(caster, target, skill_data),
        "complete_wound_healing": lambda: _complete_wound_healing(caster, target),
        "constitution_boost": lambda: _constitution_boost(caster),
        "consume_all_shadows": lambda: _consume_all_shadows(caster),
        "corruption_risk": lambda: _corruption_risk(caster),
        "cosmic_insight": lambda: _cosmic_insight(caster),
        "courage_boost": lambda: _courage_boost(caster),
        "craft_gadget": lambda: _craft_gadget(caster),
        "critical_damage_up": lambda: _critical_damage_up(caster),
        "critical_rate_up": lambda: _critical_rate_up(caster),
        "damage_stack": lambda: _damage_stack(caster),
        "dark_magic": lambda: _dark_magic(caster, target, skill_data),
        "deep_recovery": lambda: _deep_recovery(caster),
        "defensive_bonus": lambda: _defensive_bonus(caster),
        "deploy_robot": lambda: _deploy_robot(caster),
        "dimension_storm": lambda: _dimension_storm(caster, target, skill_data),
        "dimensional_shift": lambda: _dimensional_shift(caster),
        # 네 번째 배치 효과들
...
def _chronos_blessing(caster, target=None, skill_data=None):
    ...
    try:
        if hasattr(caster, 'status_manager'):
            from game.new_skill_system import StatusEffect
caster.status_manager.add_status(StatusEffect("시간_조작", "시간_조작", 8, 30))
            from game.new_skill_system import StatusEffect
caster.status_manager.add_status(StatusEffect("시간_가속", "시간_가속", 8, 25))
...
def _constitution_boost(caster, target=None, skill_data=None):
    ...
    try:
        if hasattr(caster, 'status_manager'):
            from game.new_skill_system import StatusEffect
caster.status_manager.add_status(StatusEffect("체질_강화", "체질_강화", 15, 25))  # 체력과 저항력 25% 증가
...
def _corruption_risk(caster, target=None, skill_data=None):
    ...
    try:
        if hasattr(caster, 'status_manager'):
            from game.new_skill_system import StatusEffect
caster.status_manager.add_status(StatusEffect("타락의_힘", "타락의_힘", 10, 50))  # 공격력 50% 증가
...
def _cosmic_insight(caster, target=None, skill_data=None):
    ...
    try:
        if hasattr(caster, 'status_manager'):
            from game.new_skill_system import StatusEffect
caster.status_manager.add_status(StatusEffect("우주적_통찰", "우주적_통찰", 12, 40))
            from game.new_skill_system import StatusEffect
caster.status_manager.add_status(StatusEffect("진실_시야", "진실_시야", 12, 100))
...
def _courage_boost(caster, target=None, skill_data=None):
    ...
    try:
        if hasattr(caster, 'status_manager'):
            ...
            from game.new_skill_system import StatusEffect
caster.status_manager.add_status(StatusEffect("용기", "용기", 8, 30))
...
def _critical_damage_up(caster, target=None, skill_data=None):
    ...
    try:
        if hasattr(caster, 'status_manager'):
            from game.new_skill_system import StatusEffect
caster.status_manager.add_status(StatusEffect("크리티컬_피해_증가", "크리티컬_피해_증가", 8, 40))
...
def _critical_rate_up(caster, target=None, skill_data=None):
    ...
    try:
        if hasattr(caster, 'status_manager'):
            from game.new_skill_system import StatusEffect
caster.status_manager.add_status(StatusEffect("크리티컬_확률_증가", "크리티컬_확률_증가", 8, 25))
        print(f"{caster.name}의 크리티컬 확률이 증가했습니다!")
        return True
    except Exception as e:
...
def _damage_stack(caster, target=None, skill_data=None):
    ...
    try:
        if hasattr(caster, 'status_manager'):
            ...
            stack_bonus = caster.damage_stack * 10
            caster.status_manager.add_status('피해_누적', 10, stack_bonus)
        print(f"{caster.name}의 피해 누적이 증가했습니다! (스택: {getattr(caster, 'damage_stack', 1)})")
        return True
    except Exception as e:
...
def _deep_recovery(caster, target=None, skill_data=None):
    ...
    try:
        if hasattr(caster, 'status_manager'):
            from game.new_skill_system import StatusEffect
caster.status_manager.add_status(StatusEffect("깊은_회복", "깊은_회복", 20, 30))  # 20턴간 턴마다 30 회복
...
def _defensive_bonus(caster, target=None, skill_data=None):
    ...
    try:
        if hasattr(caster, 'status_manager'):
            from game.new_skill_system import StatusEffect
caster.status_manager.add_status(StatusEffect("방어_보너스", "방어_보너스", 10, 35))
...
def _deploy_robot(caster, target=None, skill_data=None):
    ...
    try:
        if hasattr(caster, 'status_manager'):
            from game.new_skill_system import StatusEffect
caster.status_manager.add_status(StatusEffect("전투_로봇", "전투_로봇", 25, 60))  # 25턴간 자동 공격
...
def _dimension_storm(caster, target, skill_data):
    ...
    try:
        if target and skill_data:
            ...
            if hasattr(target, 'status_manager'):
                from game.new_skill_system import StatusEffect
target.status_manager.add_status(StatusEffect("공간_왜곡", "공간_왜곡", 5, 30))
...
def _dimensional_shift(caster, target=None, skill_data=None):
    ...
    try:
        if hasattr(caster, 'status_manager'):
            from game.new_skill_system import StatusEffect
caster.status_manager.add_status(StatusEffect("차원_이동", "차원_이동", 3, 0))  # 3턴간 공격 받지 않음
...
def _draconic_might(caster, target=None, skill_data=None):
    ...
    try:
        if hasattr(caster, 'status_manager'):
            from game.new_skill_system import StatusEffect
caster.status_manager.add_status(StatusEffect("용의_힘", "용의_힘", 12, 60))  # 모든 능력치 60% 증가
            from game.new_skill_system import StatusEffect
caster.status_manager.add_status(StatusEffect("용의_비늘", "용의_비늘", 12, 40))  # 방어력 추가 증가
...
def _earth_elementalist(caster, target=None, skill_data=None):
    ...
    try:
        if hasattr(caster, 'status_manager'):
            from game.new_skill_system import StatusEffect
caster.status_manager.add_status(StatusEffect("대지_마스터리", "대지_마스터리", 15, 45))
            from game.new_skill_system import StatusEffect
caster.status_manager.add_status(StatusEffect("토속성_강화", "토속성_강화", 15, 50))
...
def _earth_power(caster, target=None, skill_data=None):
    ...
    try:
        if hasattr(caster, 'status_manager'):
            from game.new_skill_system import StatusEffect
caster.status_manager.add_status(StatusEffect("대지의_힘", "대지의_힘", 10, 35))
            from game.new_skill_system import StatusEffect
caster.status_manager.add_status(StatusEffect("견고함", "견고함", 10, 30))
...
def _earth_resonance(caster, target=None, skill_data=None):
    ...
    try:
        if hasattr(caster, 'status_manager'):
            from game.new_skill_system import StatusEffect
caster.status_manager.add_status(StatusEffect("대지_공명", "대지_공명", 12, 25))
...
def _electric_boost(caster, target=None, skill_data=None):
    ...
    try:
        if hasattr(caster, 'status_manager'):
            from game.new_skill_system import StatusEffect
caster.status_manager.add_status(StatusEffect("전기_강화", "전기_강화", 8, 40))
            from game.new_skill_system import StatusEffect
caster.status_manager.add_status(StatusEffect("번개_친화", "번개_친화", 8, 30))
...
def _electric_field(caster, target=None, skill_data=None):
    ...
    try:
        if hasattr(caster, 'status_manager'):
            from game.new_skill_system import StatusEffect
caster.status_manager.add_status(StatusEffect("전기장", "전기장", 15, 25))  # 적 접근 시 피해
...
def _elemental_barrier(caster, target=None, skill_data=None):
    ...
    try:
        if hasattr(caster, 'status_manager'):
            from game.new_skill_system import StatusEffect
caster.status_manager.add_status(StatusEffect("원소_장벽", "원소_장벽", 5, 100))  # 5턴간 원소 공격 무효
...
def _elemental_mastery(caster, target=None, skill_data=None):
    """원소 마스터리 - 모든 원소 능력 숙련도 증가"""
    try:
        if hasattr(caster, 'status_manager'):
            from game.new_skill_system import StatusEffect
caster.status_manager.add_status(StatusEffect("원소_마스터리", "원소_마스터리", 20, 50))
        print(f"{caster.name}의 원소 마스터리가 증가했습니다!")
        return True
    except Exception as e:
        print(f"원소 마스터리 효과 적용 중 오류: {e}")
        return False
...
def _elemental_overload(caster, target=None, skill_data=None):
    ...
    try:
        if hasattr(caster, 'status_manager'):
            from game.new_skill_system import StatusEffect
caster.status_manager.add_status(StatusEffect("원소_과부하", "원소_과부하", 6, 80))  # 강력하지만 짧은 지속시간
...
def _energy_boost(caster, target=None, skill_data=None):
    ...
    try:
        if hasattr(caster, 'status_manager'):
            from game.new_skill_system import StatusEffect
caster.status_manager.add_status(StatusEffect("에너지_증진", "에너지_증진", 12, 50))
...
def _energy_overload(caster, target=None, skill_data=None):
    ...
    try:
        if hasattr(caster, 'status_manager'):
            from game.new_skill_system import StatusEffect
caster.status_manager.add_status(StatusEffect("에너지_과부하", "에너지_과부하", 8, 100))  # 최대 MP 증가
            # MP 과소모 위험
            from game.new_skill_system import StatusEffect
caster.status_manager.add_status(StatusEffect("과부하_위험", "과부하_위험", 8, 0))
...
def _enhanced_accuracy(caster, target=None, skill_data=None):
    ...
    try:
        if hasattr(caster, 'status_manager'):
            from game.new_skill_system import StatusEffect
caster.status_manager.add_status(StatusEffect("정확도_향상", "정확도_향상", 10, 30))
            from game.new_skill_system import StatusEffect
caster.status_manager.add_status(StatusEffect("집중", "집중", 10, 20))
...
def _enhanced_luck(caster, target=None, skill_data=None):
    ...
    try:
        if hasattr(caster, 'status_manager'):
            from game.new_skill_system import StatusEffect
caster.status_manager.add_status(StatusEffect("행운_증진", "행운_증진", 15, 40))
...
def _enhanced_reflexes(caster, target=None, skill_data=None):
    ...
    try:
        if hasattr(caster, 'status_manager'):
            from game.new_skill_system import StatusEffect
caster.status_manager.add_status(StatusEffect("반사신경_강화", "반사신경_강화", 12, 35))
            from game.new_skill_system import StatusEffect
caster.status_manager.add_status(StatusEffect("반격_확률", "반격_확률", 12, 25))
...
def _eternal_flame(caster, target=None, skill_data=None):
    ...
    try:
        if hasattr(caster, 'status_manager'):
            from game.new_skill_system import StatusEffect
caster.status_manager.add_status(StatusEffect("영원한_불꽃", "영원한_불꽃", 25, 30))
            caster.status_manager.remove_status('빙결')
