...
# StorySystem 클래스에 추가 메서드들 (호환성용)
def show_story(story_type: str = "opening"):
    """스토리 표시 (호환성 메서드)"""
    story_system = StorySystem()
    if story_type == "opening":
        story_system.show_opening_story()
    elif story_type == "job_selection":
        segments = story_system.get_job_selection_intro()
        story_system.display_story_with_typing_effect(segments)
    elif story_type == "party_formation":
        segments = story_system.get_party_formation_story()
        story_system.display_story_with_typing_effect(segments)
    else:
        print(f"알 수 없는 스토리 타입: {story_type}")


def trigger_event(event_type: str, **kwargs):
    """이벤트 트리거 (호환성 메서드)"""
    story_system = StorySystem()
    if event_type == "sephiroth_encounter":
        story_system.set_sephiroth_encountered(True)
        segments = story_system.get_sephiroth_encounter_story()
        story_system.display_story_with_typing_effect(segments)
...
def get_story(story_id: str):
    ...
    story_system = StorySystem()
    if story_id == "opening":
        return story_system.get_opening_story()
    elif story_id == "job_selection":
        return story_system.get_job_selection_intro()
    elif story_id == "party_formation":
        return story_system.get_party_formation_story()
    elif story_id == "first_adventure":
        return story_system.get_first_adventure_story()
    else:
        return []


# StorySystem 클래스에 메서드 추가
StorySystem.show_story = lambda self, story_type="opening": show_story(story_type)
StorySystem.trigger_event = lambda self, event_type, **kwargs: trigger_event(event_type, **kwargs)
StorySystem.get_story = lambda self, story_id: get_story(story_id)
