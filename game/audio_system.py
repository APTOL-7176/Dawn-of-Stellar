...
class UnifiedAudioSystem:
    ...
    def __init__(self, debug_mode: bool = False):
        ...
        self.bgm_path = self.base_path / "sounds" / "bgm"
        self.sfx_path = self.base_path / "sounds" / "sfx"
    ...
    def _initialize_pygame(self):
        """Pygame 믹서만 초기화 (창 생성 방지)"""
        try:
            # pygame.init() 대신 mixer만 초기화 (창 생성 방지)
    ...
    def change_bgm(self, bgm_type):
        """BGM 변경 (기존 BGM 중지 후 새 BGM 재생)"""
        self.stop_bgm()
        self.play_bgm(bgm_type)
        if self.debug_mode:
            print(f"🔄 BGM 변경: {bgm_type}")
