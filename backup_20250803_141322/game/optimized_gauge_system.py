...
class OptimizedGaugeSystem:
    ...
    def create_status_line(character, current_char=None, all_characters=None) -> str:
        ...
        # ATB 게이지 처리 - 값 범위 안정화
        atb_gauge = max(0, min(10000, getattr(character, 'atb_gauge', 0)))  # 0-10000 범위 강제
        atb_ready_threshold = 10000  # ATB_MAX 10000에 맞춤
    ...
    def show_optimized_enemy_status(enemies) -> str:
        ...
        for enemy in enemies:
            if enemy.is_alive:
                ...
                if atb_gauge >= 10000:  # ATB_MAX 10000에 맞춤
                    atb_status = f"{Color.YELLOW}⚡ READY{Color.RESET}"
                    atb_bar = OptimizedGaugeSystem.create_clean_gauge(10000, 10000, 10, "atb", hp_ratio, mp_ratio, is_casting, atb_speed_state)
                else:
                    atb_percent = int((atb_gauge / 10000) * 100)  # 10000 기준으로 변경
                    # ATB 게이지 색상 결정
                    if is_casting:
                        atb_color = Color.BRIGHT_MAGENTA
                    elif atb_speed_state == "fast":
                    ...
                    atb_bar = OptimizedGaugeSystem.create_clean_gauge(atb_gauge, 10000, 10, "atb", hp_ratio, mp_ratio, is_casting, atb_speed_state)
