...
class EnhancedEncounter:
    ...
    def execute_interaction(self, interaction: Dict, party: List[Character], floor: int) -> Dict[str, Any]:
        ...
        if success:
            result = self._handle_success(interaction_type, party, floor)
        else:
            result = self._handle_failure(interaction_type, party, floor)
        
        # 결과 메시지 출력 후 대기
        if result.get("message"):
            print(f"\n{result['message']}")
            import time
            time.sleep(2.0)  # 2초 대기로 결과를 읽을 시간 제공
