    ...
    def _select_character_passives(self, character: Character) -> bool:
        ...
        while True:
            ...
            if hasattr(character, 'active_traits') and character.active_traits:
                print(f"\n{GREEN}현재 선택된 특성:{RESET}")
                for i, trait in enumerate(character.active_traits, 1):
                    print(f"  {i}. {CYAN}{trait.name}{RESET} - {trait.description}")
                print(f"{YELLOW}선택된 특성 개수: {len(character.active_traits)}/2{RESET}")
            else:
                print(f"\n{YELLOW}아직 선택된 특성이 없습니다. (0/2){RESET}")
            
            character.display_available_traits()
            
            try:
                print(f"{WHITE}선택할 특성 번호를 입력하세요 (1-5, 최대 2개, 쉼표로 구분):")
                print(f"예: 1,3 또는 2,5")
                print(f"{YELLOW}명령어:")
                print(f"  • 'cancel'/'취소' : 모든 선택 초기화")
                print(f"  • 'remove 1'/'제거 1' : 첫 번째 선택된 특성 제거")
                print(f"  • 'done'/'완료' : 현재 선택으로 확정{RESET}")
                ...
                # 명령어 처리
                if user_input.lower() in ['cancel', '취소', 'c']:
                    if hasattr(character, 'active_traits') and character.active_traits:
                        character.active_traits = []
                        print(f"{YELLOW}✓ {character.name}의 패시브 선택이 초기화되었습니다.{RESET}")
                        continue
                    else:
                        print(f"{RED}취소할 선택이 없습니다.{RESET}")
                        continue
                
                # 완료 명령어 처리
                elif user_input.lower() in ['done', '완료', 'd']:
                    if hasattr(character, 'active_traits') and character.active_traits:
                        if len(character.active_traits) >= 1:  # 최소 1개는 선택해야 함
                            ...
                            print(f"{RED}최소 1개의 특성을 선택해야 합니다.{RESET}")
                            continue
                    else:
                        print(f"{RED}선택된 특성이 없습니다.{RESET}")
                        continue
                
                # 개별 제거 명령어 처리
                elif user_input.lower().startswith(('remove ', '제거 ', 'r ')):
                    try:
                        # "remove 1" 또는 "제거 1" 형태에서 숫자 추출
                        parts = user_input.split()
                        if len(parts) >= 2:
                            remove_index = int(parts[1]) - 1
                            if hasattr(character, 'active_traits') and character.active_traits:
                                if 0 <= remove_index < len(character.active_traits):
                                    removed_trait = character.active_traits.pop(remove_index)
                                    print(f"{YELLOW}✓ '{removed_trait.name}' 특성이 제거되었습니다.{RESET}")
                                    continue
                                else:
                                    print(f"{RED}잘못된 인덱스입니다. 1~{len(character.active_traits)} 범위로 입력하세요.{RESET}")
                                    continue
                            else:
                                print(f"{RED}제거할 특성이 없습니다.{RESET}")
                                continue
                        else:
                            print(f"{RED}제거할 특성 번호를 입력하세요. 예: remove 1{RESET}")
                            continue
                    except ValueError:
                        print(f"{RED}올바른 번호를 입력하세요. 예: remove 1{RESET}")
                        continue
