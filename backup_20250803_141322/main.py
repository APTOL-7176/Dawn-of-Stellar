...
class DawnOfStellarGame:
    ...
    def __init__(self):
        ...
        try:
            ...
            print("🔥 강화된 조우, 특성, 밸런스 시스템 활성화!")
        except ImportError as e:
            print(f"⚠️ 강화 시스템 일부 로드 실패: {e}")
...
{bright_yellow('🎯 처음이시라면 "AI 파트너 모드"를 추천합니다!')}"""
    ...
    def _restore_world_state(self, world_state):
        ...
        try:
            ...
            if not hasattr(self, 'world') or not self.world:
                ...
                # 여기서 월드 생성 로직을 호출해야 함
                # 실제 구현은 게임의 월드 시스템에 따라 달라짐
        ...
        try:
            ...
            for item_info in items_data:
                try:
                    ...
                    if item_data:
                        # 실제 아이템 생성 로직은 게임의 아이템 시스템에 따라 달라짐
                        restored_item = {
                            'x': item_info.get('x', 0),
                            'y': item_info.get('y', 0),
                            'item': item_data  # 간단한 버전
                        }
                        restored_items.append(restored_item)
                except Exception as item_error:
                    print(f"⚠️ 아이템 복원 실패: {item_error}")
                    continue
            
            if hasattr(self.world, 'items_on_ground'):
                self.world.items_on_ground = restored_items
                print(f"💰 바닥 아이템 복원: {len(restored_items)}개")
        except Exception as e:
        ...
        try:
            ...
            for room_info in room_data:
                try:
                    ...
                    restored_rooms.append(room)
                except Exception as room_error:
                    print(f"⚠️ 방 복원 실패: {room_error}")
                    continue
            
            if hasattr(self.world, 'rooms'):
                self.world.rooms = restored_rooms
                print(f"🏠 방 정보 복원: {len(restored_rooms)}개")
        except Exception as e:
    ...
    def restore_game_state(self, game_state):
        ...
        try:
            ...
            for member_data in party_data:
                # GameStateSerializer를 사용하여 캐릭터 복원
                try:
                    if SAVE_SYSTEM_AVAILABLE:
                        from game.save_system import GameStateSerializer
                        character = GameStateSerializer.deserialize_character(member_data)
                        print(f"🔄 {character.name} GameStateSerializer로 복원 중...")
                    else:
                        # 폴백: 기본 방식
    ...
    def handle_enhanced_encounter(self, encounter_result):
        ...
        try:
            ...
            print(f"\n{bright_cyan('='*50)}")
            if success:
                print(f"{bright_green('✅ 조우 결과')}")
            else:
                print(f"{bright_red('❌ 조우 결과')}")
    ...
    def _handle_menu_state(self):
        ...
        if choice == 'q' or choice == 'Q':
            ...
            try:
                ...
                training_room.enter_training_room()
