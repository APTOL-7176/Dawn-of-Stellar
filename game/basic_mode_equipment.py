...
class BasicModeEquipmentUI:
    ...
    def show_auto_equip_menu(self, character, inventory_items: List = None):
        ...
        if choice == '1':
            ...
            self._optimize_party_equipment(inventory_items)
        elif choice == '5':
            print("❌ 취소되었습니다.")
        else:
            print("❌ 잘못된 선택입니다.")
    ...
    def _optimize_party_equipment(self, inventory_items):
        """파티 전체 장비 최적화"""
        try:
            # 파티 정보 가져오기 (게임 인스턴스에서)
            import main
            if hasattr(main, 'game_instance') and main.game_instance and hasattr(main.game_instance, 'party') and main.game_instance.party:
                party = main.game_instance.party
            else:
