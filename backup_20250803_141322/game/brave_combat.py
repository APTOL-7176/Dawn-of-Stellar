...
from .ui_animations import animate_hp_change, animate_mp_change, animate_brv_change, show_animated_damage, show_animated_healing, show_status_change_animation
...
class BraveCombatSystem:
    ...
    def __init__(self, audio_system=None, sound_manager=None):
        ...
        # 애니메이션 시스템
        self.animation_duration = 1.5  # 1.5초 애니메이션
        self.animation_fps = 60  # 60 FPS로 부드럽게
        self.animation_active = False
        
        # 자동 전투 모드
        self.auto_battle = False
        self.auto_battle_delay = 0.5  # 자동 전투 시 0.5초 딜레이
    ...
    def animate_value_change(self, character: Character, stat_type: str, old_value: int, new_value: int, party: List[Character], enemies: List[Character]):
        ...
        if stat_type.lower() == 'hp':
            # HP 애니메이션 시작
            animate_hp_change(character, old_value, new_value)
            character.current_hp = new_value
            
            # 데미지/회복에 따른 효과 표시
            if new_value < old_value:
                damage = old_value - new_value
                show_animated_damage(character.name, damage, is_critical=False)
            ...
            # MP 애니메이션 시작
            animate_mp_change(character, old_value, new_value)
            character.current_mp = new_value
            
        elif stat_type.lower() == 'brv':
            # BRV 애니메이션 시작
            animate_brv_change(character, old_value, new_value)
        ...
        while True:
            ...
            while attempts < max_attempts:
                ...
                if action_order:
                    break
                attempts += 1
                time.sleep(0.12)  # ATB 업데이트 간 딜레이 추가 증가 (120ms)
    ...
    def player_turn(self, character: Character, party: List[Character], enemies: List[Character]):
        ...
        while True:
            try:
                ...
                combined_status = party_status + "\n" + enemy_status
                
                # 상태창과 메뉴를 통합하여 표시 (타이틀 없음)
                action_menu = create_simple_menu(
                    "", 
                    action_options, 
                    action_descriptions, 
                    clear_screen=True,
                    extra_content=combined_status
                )
                choice = action_menu.run()
                
                if choice == 0:  # Brave 공격
                    if self.brave_attack_menu(character, enemies):
                        # 액션 완료 후 소프트 클리어 (화면 덮어쓰기 준비)
                        time.sleep(0.5)  # 결과 확인 시간
                        break
                elif choice == 1:  # HP 공격
    ...
    def _auto_battle_action(self, character: Character, party: List[Character], enemies: List[Character]):
        ...
        time.sleep(self.auto_battle_delay)
    ...
    def execute_brave_attack(self, attacker: Character, target: Character):
        ...
        # BRV 피해량 2.5배 증가 (전투 속도 개선)
        brave_damage = int(brave_damage * 25)  # 제거
        ...
        # Brave 변화 애니메이션
        if hasattr(self, '_current_party') and hasattr(self, '_current_enemies'):
            self.animate_value_change(attacker, 'BRV', gained_brave, attacker.brave_points, self._current_party, self._current_enemies)
        else:
    ...
    def _execute_hp_attack_on_target(self, attacker: Character, target: Character, skill, consume_brave: bool = True):
        ...
        # HP 애니메이션 적용 (파티/적군 리스트 가져오기 위해 임시로 처리)
        # TODO: 메서드 시그니처 개선 필요
        if hasattr(self, '_current_party') and hasattr(self, '_current_enemies'):
            self.animate_value_change(target, 'HP', old_hp, new_hp, self._current_party, self._current_enemies)
        ...
        if consume_brave:
            ...
            # Brave 변화 애니메이션 표시
            if hasattr(self, '_current_party') and hasattr(self, '_current_enemies'):
                self.animate_value_change(attacker, 'BRV', old_brave, attacker.brave_points, self._current_party, self._current_enemies)
            else:
    ...
    def _get_party_status_string(self, current_char: Character, party: List[Character], enemies: List[Character]) -> str:
        ...
        for member in party:
            ...
            if member.is_alive:
                ...
                if hasattr(member, 'is_casting') and member.is_casting:
                    ...
                    atb_bar = self.create_beautiful_atb_gauge(int(casting_progress * 100), 100, 15, True)
                ...
                # 간단한 1줄 형식으로 출력 (컴팩트 버전)
                status_lines.append(f"  {status_icon} {class_icon} Lv.{getattr(member, 'level', 1)} {name_color}{member.name}{Color.RESET}")
                status_lines.append(f"💚 HP: {member.current_hp} / {member.max_hp} {Color.WHITE}{{{hp_bar}}}{Color.RESET} | 💙 MP: {member.current_mp} / {member.max_mp} {Color.WHITE}{{{mp_bar}}}{Color.RESET} | {brv_color}⚡ BRV: {brave_points}{Color.RESET} |")
                status_lines.append(f"⏳ TIME: {Color.WHITE}{{{atb_bar}}}{Color.RESET} {atb_display} | SPD: {spd_color}{member_speed}{Color.RESET}{casting_status}")
            else:
                # 전투불능 상태 표시
        ...
        if alive_enemies:
            ...
            for enemy in alive_enemies:
                ...
                status_lines.append(f"  {atb_icon} {Color.WHITE}{{{atb_bar}}}{Color.RESET} {atb_display} | SPD: {getattr(enemy, 'speed', 50)}")
    ...
    def _update_atb_instant(self, all_combatants: List[Character], atb_settings: dict):
        ...
        # ATB 게이지 충전 속도 - 속도 기반으로 개별 계산 (부드러운 증가)
        base_increase = 250  # 빠른 전투를 유지하되 부드럽게
        ...
        if atb_updates and hasattr(self, '_current_party') and hasattr(self, '_current_enemies'):
            ...
            if significant_changes:
                # 잠시 대기 후 안정적인 상태에서 화면 갱신
                import time
                time.sleep(0.1)  # 100ms 대기로 안정화
    ...
    def _update_atb_with_animation(self, all_combatants: List[Character], atb_settings: dict):
        """ATB 애니메이션과 함께 업데이트 - 60FPS로 부드럽게"""
        import time
        import os
        
        speed_multiplier = atb_settings.get("update_speed", 1.0)
        frame_delay = 1.0/120  # 120 FPS로 더욱 부드럽게 (1/120초)
        ...
        base_increase = 5  # 기본 증가량을 더 낮게 설정
    ...
    def _calculate_brv_damage(self, skill, caster, target, brv_power):
        ...
        # 스킬 정보 수집
        skill_info = {
            "name": skill.get("name", "기본 공격") if isinstance(skill, dict) else getattr(skill, 'name', "기본 공격"),
            "brv_power": brv_power,
            "type": "BRV"
        }
        ...
        calculation_steps["1. 공격자_물리공격력"] = base_attack
        calculation_steps["2. 대상_물리방어력"] = target_defense
        
        # 2. 기본 데미지 계산 (GameBalance 사용)
        base_damage = GameBalance.calculate_brave_damage(base_attack, target_defense, 1.0, caster)
        calculation_steps["3. 기본_BRV_데미지"] = base_damage
        
        # 3. 스킬 배율 적용
        skill_damage = int(base_damage * (brv_power / 100.0))
        calculation_steps["4. 스킬_배율_적용"] = f"{brv_power}% → {skill_damage}"
        
        # 4. 기본 배율 적용 (1.5배)
        enhanced_damage = int(skill_damage * 1.5)
        calculation_steps["5. 기본_배율_1.5x"] = enhanced_damage
        
        # 5. 랜덤 변수 적용
        import random
        random_factor = random.uniform(0.9, 1.1)
        random_damage = int(enhanced_damage * random_factor)
        calculation_steps["6. 랜덤_변수"] = f"{random_factor:.3f} → {random_damage}"
        
        # 6. 특성 효과 적용 (사용 가능한 경우)
        final_damage = random_damage
        if ENHANCED_SYSTEMS_AVAILABLE and self.trait_processor:
            # 특성 효과로 공격 데미지 증폭
            trait_enhanced = apply_trait_effects_to_damage(caster, target, random_damage, "physical")
            calculation_steps["7. 특성_효과"] = f"{random_damage} → {trait_enhanced}"
            
            # 상대적 밸런스 적용
            if self.balance_system:
                balanced_damage = calculate_balanced_damage(
                    caster.physical_attack, 
                    target.physical_defense, 
                    trait_enhanced / random_damage if random_damage > 0 else 1.0
                )
                calculation_steps["8. 밸런스_조정"] = f"{trait_enhanced} → {balanced_damage}"
                final_damage = balanced_damage
            else:
                final_damage = trait_enhanced
                
            # 특수 능력 발동 (불린 타입 특성들)
            from .trait_integration_system import trigger_special_abilities
            special_effects = trigger_special_abilities(caster, target)
            if special_effects:
                calculation_steps["9. 특수_능력"] = special_effects[:2]
                for effect_msg in special_effects[:2]:  # 최대 2개까지만 표시
                    print(f"  {effect_msg}")
        else:
            # 기존 시스템 fallback
            trait_enhanced = trait_integrator.apply_attack_trait_effects(caster, target, random_damage)
            calculation_steps["7. 특성_효과_기본"] = f"{random_damage} → {trait_enhanced}"
            
            # 무기 숙련도 효과 적용
            if hasattr(caster, 'equipped_weapon') and caster.equipped_weapon:
                weapon_type = getattr(caster.equipped_weapon, 'weapon_type', 'sword')
                weapon_enhanced = trait_integrator.apply_weapon_mastery_effects(caster, weapon_type, trait_enhanced)
                calculation_steps["8. 무기_숙련도"] = f"{trait_enhanced} → {weapon_enhanced}"
                final_damage = weapon_enhanced
            else:
                final_damage = trait_enhanced
        
        # 7. 최종 제한 적용
        original_damage = final_damage
        final_damage = max(1, min(99999, int(final_damage)))
        if original_damage != final_damage:
            calculation_steps["9. 데미지_제한"] = f"{original_damage} → {final_damage}"
        
        calculation_steps["final_damage"] = final_damage
    ...
    def _calculate_hp_damage_from_skill(self, skill, caster, target, hp_power):
        """🔥 스킬의 HP 데미지 계산 - 강화된 시스템 통합"""
        # 기본 HP 데미지 계산
        caster_attack = getattr(caster, 'attack', 100)
        target_defense = getattr(target, 'defense', 100)
        base_damage = int(caster_attack * (hp_power / 100) * (100 / (100 + target_defense)))
